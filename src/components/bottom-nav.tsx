...
export function BottomNav({ notificationUnread, onOpenTools, toolsActive }: Props) {
  ...
  return (
    <nav
      aria-label="Primary"
      className="fixed inset-x-0 bottom-0 z-40 grid grid-cols-4 border-t border-white/[0.08] bg-[#1a1a1a] pb-[max(1.25rem,env(safe-area-inset-bottom))] pt-5 sm:hidden"
    >
      {items.map((item) => {
